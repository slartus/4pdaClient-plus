...
class Http private constructor(context: Context, appName: String, appVersion: String) {
    ...
    init {
        val builder = newClientBuilder(context)
    ...
    private fun buildRequestHeaders(userAgent: String = this.userAgent): Headers {
        return buildRequestHeaders(userAgent, false)
    }
    ...
    fun callRequest(url: String) = callRequest(url, false)
    private fun callRequest(url: String, desktopVersion: Boolean = false): Response {
        try {
            if (desktopVersion)
            ...
            return client.newCall(request, desktopVersion).execute()
        } finally {
            if (desktopVersion)
    ...
    fun performGetFull(url: String): AppResponse {
        val response = callRequest(url, true)
        val body = response.body?.string()
        return AppResponse(url, response.request.url.toString(), body ?: "", response.code())
    }
    ...
    fun postMultipart(url: String, values: List<Pair<String, String>>): AppResponse {
        ...
        try {
            val response = client.newCall(request, desktopVersion = false).execute()
    ...
    fun performPostDesktop(
        ...
        try {
            val response = client.newCall(request, true).execute()
            if (!response.isSuccessful) throw HttpException("Unexpected code $response")
    ...
    fun performPost(
        ...
        try {
            val response = client.newCall(request, false).execute()
            if (!response.isSuccessful) throw HttpException("Unexpected code $response")
    ...
    fun performPost(url: String, json: String): AppResponse {
        ...
        try {
            ...
            return AppResponse(url, response.request.url.toString(), body2 ?: "", response.code())
        } catch (ex: Exception) {
            throw HttpException(ex)
        }
    ...
    companion object {
        ...
        @JvmStatic
        fun newClientBuilder(context: Context): OkHttpClient.Builder {
            ...
            return OkHttpClient.Builder()
                ...
                .cookieJar(
                    JavaNetCookieJar(
                        CookieManager(
                            PersistentCookieStore.getInstance(context),
                            ACCEPT_ALL
                        )
                    )
                )
        ...
        fun OkHttpClient.newCall(request: Request, desktopVersion: Boolean): Call {
            PersistentCookieStore.desktopVersion = desktopVersion
