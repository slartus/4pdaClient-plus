...
abstract class TopicsListFragment : BaseTaskListFragment() {
    ...
    protected abstract fun loadTopics(
        client: Client?,
        listInfo: ListInfo?
    ): ArrayList<out IListItem?>
    ...
    private fun showLinkMenu(context: Context?, topic: IListItem) {
        ...
        ExtUrl.addUrlSubMenu(
            mHandler, context, list,
            TopicUtils.getTopicUrl(
                topic.id.toString(), TopicUtils.getOpenTopicArgs(
                    topic.id,
                    listName
                ) ?: ""
            ), topic.id.toString(),
    ...
    private fun runTopicActionAsync(
        topic: Topic,
        actionAsync: () -> String?,
    ...
    private fun togglePinAsync(topic: FavTopic) {
        runTopicActionAsync(topic,
            {
                TopicApi.pinFavorite(
                    Client.getInstance(),
                    topic.id,
                    if (topic.isPinned) TopicApi.TRACK_TYPE_UNPIN else TopicApi.TRACK_TYPE_PIN
                )
            },
            {
    ...
    private fun configureOptionsMenu(
        context: Context?, mHandler: Handler?, optionsMenus: MutableList<MenuItemAction>,
        listItem: IListItem
    ) {
        ...
        if (Client.getInstance().logined && !topic.isInProgress) {
            ...
            val title =
                if (isFavotitesList) context!!.getString(R.string.change_subscription) else context!!.getString(
    ...
    private fun showSaveNavigateActionDialog(
        topic: IListItem,
        selectedAction: CharSequence,
        params: String
    ) {
        val context = context
        if (context != null) {
            showSaveNavigateActionDialog(
                context, String.format("%s.navigate_action", listName),
    ...
    private fun tryCreatePost(topic: IListItem): Boolean {
        ...
        if (!extras.containsKey(Intent.EXTRA_STREAM) &&
            ...
            !extras.containsKey(Intent.EXTRA_HTML_TEXT)
        ) return false
        val context = context
        if (context != null) {
            newPostWithAttach(
    ...
    override fun onItemClick(adapterView: AdapterView<*>, v: View, position: Int, id: Long) {
        ...
        try {
            ...
            if (!instance.getLogined()) {
                Toast.makeText(context, "Залогиньтесь для просмотра тем форума!", Toast.LENGTH_LONG)
                    .show()
            }
            execute(
                activity!!,
                ...
                arrayOf(
                    getString(R.string.navigate_getfirstpost),
                    getString(R.string.navigate_getlastpost),
                    getString(R.string.navigate_getnewpost),
                    getString(R.string.navigate_last_url)
                ),
                arrayOf(
                    Topic.NAVIGATE_VIEW_FIRST_POST,
                    Topic.NAVIGATE_VIEW_LAST_POST,
                    Topic.NAVIGATE_VIEW_NEW_POST,
                    Topic.NAVIGATE_VIEW_LAST_URL
                ),
    ...
    private val comparator: Comparator<in IListItem>
        get() = Comparator { listItem1: IListItem?, listItem2: IListItem ->
            ...
            when (Preferences.List.getListSort(listName, Preferences.List.defaultListSort())) {
                "sortorder.desc" -> return@Comparator compareBySortOrder(
                    listItem1,
                    listItem2 as Topic,
                    -1
                )
                "sortorder.asc" -> return@Comparator compareBySortOrder(
                    listItem1,
                    listItem2 as Topic,
                    1
                )
    ...
    private fun compareBySortOrder(listItem1: Topic, listItem2: Topic, k: Int): Int {
        ...
        return if (TextUtils.isEmpty(listItem2.sortOrder)) -k else k * listItem1.sortOrder.toString()
            .compareTo(listItem2.sortOrder.toString())
    }
    ...
    private fun compareByDate(listItem1: Topic, listItem2: Topic, k: Int): Int {
        ...
        return if (listItem1.lastMessageDate == null) k else k * if (listItem1.lastMessageDate.after(
                listItem2.lastMessageDate
            )
        ) 1 else -1
    }
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        ...
        if (menu.findItem(R.id.list_settings_item) != null) menu.findItem(R.id.list_settings_item).isVisible =
            true
    }
